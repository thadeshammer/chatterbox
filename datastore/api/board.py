...
@board_routes.get("/", response_model=list[BoardRead])
async def get_all_boards_endpoint() -> list[BoardRead]:
    try:
...
async def get_boards_created_by_user_id_endpoint(user_id: str) -> list[BoardRead]:
    try:
